...
def plot_binary_outliers(dataset, col, outlier_col, reset_index):
    ...
    # Grab time values that are non outliers, grab col values that are non outliers
    ax.plot(
...
# anything outside of these lines are outliers (y is acc_x and each box plot are the labels)
...
# REST data has more outliers because it isn't normally distributed
for col in outlier_columns:
...
def mark_outliers_lof(dataset, columns, n=20):
    ...
    dataset["outlier_lof"] = outliers == -1  # -1 outlier, 1 is not an outlier
...
# Shows more outliers within data
...
# mark outliers as np.nan by iterating over columns (potential outliers) while grouped by label
for col in outlier_columns:
    for label in df["label"].unique():
        # Will group by label, generates 'col' + '_outlier' column
        dataset = mark_outliers_chauvenet(df[df["label"] == label], col)

        # replace values marked as outliers with NaN
        dataset.loc[dataset[col + "_outlier"], col] = (
            np.nan
        )  # index where outliers occur
